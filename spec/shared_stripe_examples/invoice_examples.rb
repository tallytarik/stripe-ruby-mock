...
shared_examples 'Invoice API' do
  ...
  context "retrieving upcoming invoice" do
    ...
    let(:plan)          { stripe_helper.create_plan(id: '50m', amount: 50_00, interval: 'month', currency: 'usd') }
    let(:quantity)      { 3 }
    ...
    describe 'proration' do
      ...
      [false, true].each do |with_trial|
        describe "prorating a subscription with a new plan, with_trial: #{with_trial}" do
          let(:new_monthly_plan) { stripe_helper.create_plan(id: '100m', amount: 100_00, interval: 'month', currency: 'usd') }
          let(:new_yearly_plan) { stripe_helper.create_plan(id: '100y', amount: 100_00, interval: 'year', currency: 'usd') }
          let(:plan) { stripe_helper.create_plan(id: '50m', amount: 50_00, interval: 'month', currency: 'usd') }
